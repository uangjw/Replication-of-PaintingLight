...
# Global position of the light source
gx = 0.0
gy = 0.0
...
def d_resize(x, d, fac=1.0):
    ...
    if new_min < raw_min:
        ...
        interpolation = cv2.INTER_LANCZOS4
    y = cv2.resize(x, (int(d[1] * fac), int(d[0] * fac)),
                   interpolation=interpolation)
    return y


def get_image_gradient(dist):  # 加权差分
    cols = cv2.filter2D(dist, cv2.CV_32F,
                        np.array([[-1, 0, +1], [-2, 0, +2], [-1, 0, +1]]))
    rows = cv2.filter2D(dist, cv2.CV_32F,
...
def generate_lighting_effects(stroke_density, content):
    ...
    stroke_density_scaled = (stroke_density.astype(np.float32) / 255.0).clip(
        0, 1)
    coarse_effect_cols *= (1.0 - stroke_density_scaled**2.0 + 1e-10)**0.5
    coarse_effect_rows *= (1.0 - stroke_density_scaled**2.0 + 1e-10)**0.5
    refined_result = np.stack(
        [stroke_density_scaled, coarse_effect_cols, coarse_effect_rows],
        axis=2)
